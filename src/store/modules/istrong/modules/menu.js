...
export default {
  ...
  actions: {
    ...
    asideCollapseLoad({ state, dispatch }) {
      return new Promise(async resolve => {
        // store 赋值
        state.asideCollapse = await dispatch('istrong/db/get', {
          dbName: 'sys',
          path: 'menu.asideCollapse',
    ...
    async sourceDataLoad({ state, dispatch }) {
      return new Promise(async resolve => {
        // 菜单数据源持久化
        state.sourceData = await dispatch('istrong/db/get', {
          dbName: 'database',
  ...
  mutations: {
     ...
     *
     * @param {object}} state
     * @param {Boolean} isShowAside
     */
    asideShowSet(state, isShowAside) {
